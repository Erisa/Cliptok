...
namespace Cliptok.Helpers
{
    public class ScamHelpers
    {
        ...
        public static async Task<bool> CheckAvatarsAsync(DiscordMember member)
        {
            ...
            {
                discord.Logger.LogDebug("Unnecessary avatar check skipped for {member.Id}", member.Id);
                return false;
            }
            ...
            {
                discord.Logger.LogDebug("Avatar check for {member.Id}: {httpStatus} {responseString}", member.Id, httpStatus, responseString);
            ...
            {
                discord.Logger.LogError("Avatar check for {member.Id}: {httpStatus} {responseString}", member.Id, httpStatus, responseString);
            }
