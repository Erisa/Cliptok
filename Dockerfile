FROM --platform=${BUILDPLATFORM} \
    mcr.microsoft.com/dotnet/sdk:10.0.401 AS build-env
WORKDIR /app
...
FROM mcr.microsoft.com/dotnet/sdk:10.0.401 AS git-collector
WORKDIR /out
COPY . .
RUN touch dummy.txt && \
    if [ -d .git ]; then \
...
# Build runtime image
FROM mcr.microsoft.com/dotnet/runtime:10.0.12-alpine3.24
